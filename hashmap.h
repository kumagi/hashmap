...
public:
	...
	retry:
		...
		while(target != NULL){
			if(target->kvp_.first == k){ return true; }
			target = target->next_;
		}
		return false;
	}
	...
	~hashmap(){
		for(size_t i=0; i < lock_qty_; ++i){	locks_[i].lock();	}
		for(size_t i=0; i < bucket_vector_.size(); i++){
		...
		for(size_t i=0; i < lock_qty_; ++i){	locks_[i].unlock();	}
		delete[] locks_;
	}
	void dump()const{
		for(int i=0; i < lock_qty_; ++i){	locks_[i].lock();	}
		...
		for(int i=0; i < lock_qty_; ++i){	locks_[i].unlock();	}
	}
private:
	bool buckets_extend(){
		...
		for(size_t i=0; i < lock_qty_; ++i){locked[i] = false;}
